...
export default function ReportingPage() {
  ...
  return (
    <div className="space-y-5 animate-fadeIn">
      <div>
        <h1 className="ds-h1">Reporting & Analytics</h1>
        <p className="text-[12px] font-medium text-placeholder mt-0.5">Health, trends, coverage, and agent intelligence metrics</p>
      </div>

      {/* KPI Row */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <KpiCard title="Health Score" value={`${dashboardKpis.coverageScore}%`} trend={3} icon={ShieldCheck} color="#388E3C" />
        <KpiCard title="Pass Rate" value={`${dashboardKpis.overallPassRate}%`} trend={2.1} icon={TrendingUp} color="#007AFF" />
        <KpiCard title="Flaky Rate" value={`${Math.round((dashboardKpis.flakyTests / dashboardKpis.totalRegressionTests) * 100)}%`} trend={-12} icon={AlertTriangle} color="#e65100" />
      ...
      <div className="card-base p-5">
        <h3 className="ds-h2 mb-4 flex items-center gap-2">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
          ...
          <AgentStat icon={CheckCircle2} label="Accepted" value={aa.accepted} color="#388E3C" />
          <AgentStat icon={XCircle} label="Rejected" value={aa.rejected} color="#D32F2F" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="card-base p-5 space-y-4">
            <h4 className="ds-h3">Agent Performance</h4>
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div>
        ...
        <span className="text-[12px] font-medium text-placeholder block">{label}</span>
      </div>
    </div>
  );
}
    ...
    <div>
      ...
      <div className="h-1.5 bg-[#f1f4f7] rounded-full overflow-hidden">
        <div
          ...
          style={{ width: `${pct}%`, background: pct >= 75 ? '#388E3C' : pct >= 50 ? '#e65100' : '#D32F2F' }}
        />
      </div>
    </div>
  );
